...
# NEW ANALYSIS - INFER ALPHA GLOBALLY
...
posteriors_control_sepalpha <- (control_bda$posteriors)$support

write.csv(posteriors_control_sepalpha, file = "posteriors_control_sephalpha.csv")
...
posteriors_target_sepalpha <- (target_bda$posteriors)$support

write.csv(posteriors_target_sepalpha, file = "posteriors_target_sepalpha.csv")
...
posteriors_nottarget_sepalpha <- (nottarget_bda$posteriors)$support

write.csv(posteriors_nottarget_sepalpha, file = "posteriors_nottarget_sepalpha.csv")
...
posteriors_symmetric_sepalpha <- (symmetric_bda$posteriors)$support

write.csv(posteriors_symmetric_sepalpha, file = "posteriors_symmetric_sepalpha.csv")

# NEW VISUALIZATIONS

posteriors_target_sepalpha <- read.csv("posteriors_target_sepalpha.csv")
posteriors_nottarget_sepalpha <- read.csv("posteriors_nottarget_sepalpha.csv")
posteriors_control_sepalpha <- read.csv("posteriors_control_sepalpha.csv")
posteriors_symmetric_sepalpha <- read.csv("posteriors_symmetric_sepalpha.csv")

graph_alpha <- dplyr::bind_rows(list(target=posteriors_target_sepalpha, 
                                     nottarget=posteriors_nottarget_sepalpha,
                                     control=posteriors_control_sepalpha,
                                     symmetric=posteriors_symmetric_sepalpha), .id = 'condition')
graph_alpha <- graph_alpha %>%
  select(condition,alpha) %>%
  gather(key = "condition", value = "alpha")

ggplot(graph_alpha, aes(alpha, fill = condition)) + geom_density(alpha = 0.2) +
  labs(x = "Inferred value", y = "Density") # + 
  # ggtitle("Alpha parameter estimations by experiment condition")

# Visualize params of control condition 

graph_control <- posteriors_control_sepalpha %>%
...
graph_target <- posteriors_target_sepalpha %>%
  select(cost_istarget,cost_looksliketarget,cost_nottarget) %>%
  gather(key = "parameter", value = "cost")
...
graph_nottarget <- posteriors_nottarget_sepalpha %>%
  select(cost_istarget,cost_looksliketarget,cost_nottarget) %>%
  gather(key = "parameter", value = "cost")
...
graph_symmetric <- posteriors_symmetric_sepalpha %>%
  select(cost_istarget,cost_looksliketarget,cost_nottarget) %>%
  gather(key = "parameter", value = "cost")
